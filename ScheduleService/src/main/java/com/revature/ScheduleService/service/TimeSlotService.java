...
public class TimeSlotService {
    ...
    public void generateSlotsForWindow(AvailabilityWindow window) {
        ...
        while (!currentStart.plusMinutes(30).isAfter(window.getEndTime())) {
            ...
            if (!timeSlotRepository.existsByDoctorIdAndDateAvailableAndStartTime(
                    window.getDoctorId(),
                    window.getDate(),
                    currentStart
            )) {
    ...
    public List<TimeSlot> getSlotsForDoctorDateRange(Integer doctorId, LocalDate fromDate, LocalDate toDate) {
        return timeSlotRepository.findByDoctorIdAndDateAvailableBetweenOrderByDateAvailableAscStartTimeAsc(
                doctorId,
                fromDate,
                toDate
        );
    ...
    public List<TimeSlot> getAvailableSlotsForDoctor(Integer doctorId) {
        return timeSlotRepository.findByDoctorIdAndStatusOrderByDateAvailableAscStartTimeAsc(
    ...
    public List<TimeSlot> getAvailableSlots(Integer doctorId, LocalDate date) {
        return timeSlotRepository.findByDoctorIdAndDateAvailableAndStatusOrderByStartTimeAsc(
                doctorId,
                date,
    ...
    public List<PublicTimeSlotView> getAvailableSlotsPublic(Integer doctorId, LocalDate date, LocalDate from, LocalDate to) {
        ...
        if (date != null) {
            if (doctorId != null) {
                slots = timeSlotRepository.findByDoctorIdAndDateAvailableAndStatusOrderByStartTimeAsc(
            ...
            if (doctorId != null) {
                slots = timeSlotRepository.findByDoctorIdAndDateAvailableBetweenAndStatusOrderByDateAvailableAscStartTimeAsc(
            ...
            slots = timeSlotRepository.findByDoctorIdAndStatusOrderByDateAvailableAscStartTimeAsc(
                    doctorId, TimeSlotStatus.AVAILABLE
            );
        } else {
        ...
        if (log.isDebugEnabled()) {
            ...
            if (date != null) {
                if (doctorId != null) {
                    filteredAvailable = timeSlotRepository.countByDoctorIdAndDateAvailableAndStatus(
                            doctorId, date, TimeSlotStatus.AVAILABLE
                    );
                } else {
                ...
                if (doctorId != null) {
                    filteredAvailable = timeSlotRepository.countByDoctorIdAndDateAvailableBetweenAndStatus(
                            doctorId, start, end, TimeSlotStatus.AVAILABLE
                    );
                } else {
                ...
                filteredAvailable = timeSlotRepository.countByDoctorIdAndStatus(
                        doctorId, TimeSlotStatus.AVAILABLE
                );
            }
            log.debug(
        ...
        for (TimeSlot slot : slots) {
            results.add(toPublicTimeSlotView(slot, doctorNameCache));
        }
    ...
    public List<PublicTimeSlotView> getSlotsForDoctorRange(Integer doctorId, LocalDate from, LocalDate to) {
        ...
        List<TimeSlot> slots = timeSlotRepository
                .findByDoctorIdAndDateAvailableBetweenOrderByDateAvailableAscStartTimeAsc(doctorId, start, end);
        List<PublicTimeSlotView> results = new ArrayList<>();
        Map<Integer, String> doctorNameCache = new HashMap<>();
        for (TimeSlot slot : slots) {
            results.add(toPublicTimeSlotView(slot, doctorNameCache));
        ...
        if (doctorId != null) {
            if (start != null || end != null) {
                if (status != null) {
                    ...
                    slots = timeSlotRepository.findByDoctorIdAndDateAvailableBetweenOrderByDateAvailableAscStartTimeAsc(
                            doctorId, start, end
                    );
                }
            } else if (status != null) {
                slots = timeSlotRepository.findByDoctorIdAndStatusOrderByDateAvailableAscStartTimeAsc(doctorId, status);
            } else {
                slots = timeSlotRepository.findByDoctorIdOrderByDateAvailableAscStartTimeAsc(doctorId);
            }
        } else {
            if (start != null || end != null) {
                if (status != null) {
    ...
    @Transactional
    public TimeSlot updateStatus(TimeSlot slot, TimeSlotStatus status){
    ...
    public Map<String, Object> blockBreakPeriod(Integer doctorId, LocalDate date, LocalTime startTime, LocalTime endTime) {
        List<TimeSlot> slotsInRange = timeSlotRepository
                .findByDoctorIdAndDateAvailableAndStartTimeGreaterThanEqualAndEndTimeLessThanEqualOrderByStartTimeAsc(
                        doctorId,
                        date,
                        startTime,
                        endTime
    ...
    private PublicTimeSlotView toPublicTimeSlotView(TimeSlot slot, Map<Integer, String> doctorNameCache) {
        Integer doctorIdValue = slot.getDoctorId();
        String doctorName = resolveDoctorName(doctorIdValue, doctorNameCache);
